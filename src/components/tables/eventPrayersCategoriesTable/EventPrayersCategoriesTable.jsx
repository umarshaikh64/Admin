...
import { deleteEventPrayersCategory } from "../../../features/eventPrayersCategories/eventPrayersCategoriesSlice";

const EventPrayersCategoriesTable = ({ data }) => {
  ...
  const handleNavigate = (item) => {
    navigate("/event-prayers-categories-edit", {
      state: {
        payload: item,
        type: "edit",
      },
  ...
  const handleDelete = () => {
    dispatch(deleteEventPrayersCategory({ id: categoryId }));
  };
  ...
  return (
    <div className="h-full flex flex-col justify-between">
              ...
              {currentRows?.map((category, i) => (
                <tr className=" bg-white text-blackSemi" key={i}>
                  ...
                  <td className="py-3 text-blackLowEmp text-base font-normal font-poppins">
                    {category?.categoryEnglish?.length > 30
                      ? category?.categoryEnglish?.slice(0, 30) + "..."
      ...
      <div className="w-full">
        ...
        ></Pagination>
        <ConfirmationModal title="category" handleStatus={handleDelete} />
      </div>
    </div>
  );
};

export default EventPrayersCategoriesTable;
