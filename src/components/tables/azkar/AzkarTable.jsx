...
import { deleteAzkar } from "../../../features/azkar/azkarSlice";
import { errorNotify } from "../../../util/getNotify";
import ConfirmationModal from "../../modals/ConfirmationModal";
import { Pagination } from "../../shared/pagination/Pagination";
import NoData from "../../shared/ui/NoData";
const AzkarTable = ({ data }) => {
  ...
  const [azkarId, setAzkarId] = useState(null);
  ...
  return (
    <div className="h-full flex flex-col justify-between">
              ...
              {currentRows?.map((azkar, i) => (
                <tr className=" bg-white text-blackSemi" key={i}>
                  ...
                  <td className="py-3 text-blackLowEmp text-base font-normal font-poppins">
                    {azkar?.azkarEnglish?.length > 30
                      ? azkar?.azkarEnglish?.slice(0, 30) + "..."
                      : azkar?.azkarEnglish}
                  </td>
                  <td className="py-3 text-blackLowEmp text-base font-normal font-poppins">
                    {azkar?.categoryEnglish?.length > 30
                      ? azkar?.categoryEnglish.slice(0, 30) + "..."
                      : azkar?.categoryEnglish}
                  </td>
                  <td className="py-3 text-blackLowEmp text-base font-normal font-poppins">
                    {new Date(azkar?.timestamp * 1000).toLocaleDateString({
                      year: "numeric",
                      month: "long",
                      day: "numeric",
                    })}
                  ...
                  <td className="py-3 flex items-center justify-center gap-4">
                    <button type="button" onClick={() => handleNavigate(azkar)}>
                      <svg
                        width="24"
                        height="24"
                        viewBox="0 0 24 24"
      ...
      <div className="w-full">
        ...
        ></Pagination>
        <ConfirmationModal title="azkar" handleStatus={handleDelete} />
      </div>
    </div>
  );
};

export default AzkarTable;
