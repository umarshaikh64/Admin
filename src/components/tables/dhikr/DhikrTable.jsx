...
import { deleteDhikr } from "../../../features/dhikr/dhikrSlice";

function DhikrTable({ data }) {
  ...
  const handleDelete = () => {
    dispatch(deleteDhikr({ id: dhikrId }));
  };
  ...
  return (
    <div className="h-full flex flex-col justify-between">
      <div className=" overflow-x-auto">
        <table className="table w-full table-pin-rows table-pin-cols">
          ...
          {currentRows?.length === 0 ? (
            ...
            <tbody className="">
              {currentRows?.map((dhikr, i) => (
                <tr className=" bg-white text-blackSemi" key={dhikr?._id}>
                  ...
                  <td className="py-3 text-blackLowEmp text-base font-normal font-poppins">
                    {dhikr?.dhikrEnglish?.length > 10
                      ? dhikr?.dhikrEnglish?.slice(0, 10) + "..."
                      : dhikr?.dhikrEnglish}
                  </td>
                  <td className="py-3 text-blackLowEmp text-base font-normal font-poppins">
                    {new Date(dhikr?.timestamp * 1000).toLocaleDateString({
                      year: "numeric",
                      month: "long",
                      day: "numeric",
                    })}
                  ...
                  <td className="py-3 text-blackLowEmp text-base font-normal font-poppins flex items-center justify-center gap-4">
                    <button type="button" onClick={() => handleNavigate(dhikr)}>
                      <svg
                        width="24"
                        height="24"
                        viewBox="0 0 24 24"
      ...
      <div className="w-full">
        ...
        ></Pagination>
        <ConfirmationModal title="dhikr" handleStatus={handleDelete} />
      </div>
    </div>
  );
}

export default DhikrTable;
