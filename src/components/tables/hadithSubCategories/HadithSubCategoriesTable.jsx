...
import { deleteSubCategory } from "../../../features/hadithSubCategories/hadithSubCategoriesSlice";

function HadithSubCategoriesTable({ data }) {
  ...
  const handleDelete = () => {
    dispatch(deleteSubCategory({ id: selectedItemId }));
  };

  const handleNavigate = (item) => {
    navigate("/hadith-SubCategories-edit", {
      state: {
        payload: item,
        type: "edit",
      },
        ...
        <table className="table w-full table-pin-rows table-pin-cols">
          <thead className="px-0">
            <tr className="font-bold text-3xl text-blackHigh">
              <th className="bg-tablenav text-base text-blackMediumEmp font-normal font-poppins py-4">
                Sl.
              </th>
              <th className="bg-tablenav text-base text-blackMediumEmp font-normal font-poppins py-4">
              Sub Category name
              ...
              <th className="bg-tablenav text-base text-blackMediumEmp font-normal font-poppins py-4 text-center w-[200px]">
                Actions
              </th>
            </tr>
          </thead>
          {currentRows?.length === 0 ? (
            ...
            <tbody className="">
              {currentRows?.map((subCategory, i) => (
                <tr className=" bg-white text-blackSemi" key={subCategory._id}>
                  <td className="p-4">
                  ...
                  <td className="py-3 text-blackLowEmp text-base font-normal font-poppins">
                    {subCategory?.subCategoryEnglish?.length > 20
                      ? subCategory?.subCategoryEnglish?.slice(0, 20) + "..."
                      : subCategory?.subCategoryEnglish}
                  </td>
                  <td className="py-3 text-blackLowEmp text-base font-normal font-poppins">
                    {new Date(subCategory?.timestamp * 1000).toLocaleDateString(
                      {
                        year: "numeric",
                        month: "long",
                        day: "numeric",
                  ...
                  <td className="py-3 text-blackLowEmp text-base font-normal font-poppins w-[200px] flex justify-center items-center">
                    <button
                      type="button"
                      className="me-4"
                    ...
                    <label
                      ...
                      onClick={() => setSelectedItemId(subCategory._id)}
                    >
                      <svg
                        width="24"
                        height="24"
...
export default HadithSubCategoriesTable;
