...
import { deleteEventPrayers } from "../../../features/eventPrayers/eventPrayersSlice";
const EventPrayersTable = ({ data }) => {
  ...
  const handleDelete = () => {
    dispatch(deleteEventPrayers({ id: eventId }));
  };
  ...
  return (
    <div className="h-full flex flex-col justify-between">
        ...
        <table className="table w-full table-pin-rows table-pin-cols">
          <thead className="px-0">
            <tr className="font-bold text-3xl text-blackHigh">
              ...
              <th className="bg-tablenav text-base text-blackMediumEmp font-normal font-poppins py-4 text-center">
                Actions
              </th>
            </tr>
          </thead>
              ...
              {currentRows?.map((events, i) => (
                <tr className=" bg-white text-blackSemi" key={i}>
                  ...
                  <td className="py-3 text-blackLowEmp text-base font-normal font-poppins">
                    {events?.eventPrayerEnglish?.length > 30
                      ? events?.eventPrayerEnglish?.slice(0, 30) + "..."
                      : events?.eventPrayerEnglish}
                  </td>
                  <td className="py-3 text-blackLowEmp text-base font-normal font-poppins">
                    {events?.categoryEnglish?.length > 30
                      ? events?.categoryEnglish.slice(0, 30) + "..."
                      : events?.categoryEnglish}
                  </td>
                  <td className="py-3 text-blackLowEmp text-base font-normal font-poppins">
                    {new Date(events?.timestamp * 1000).toLocaleDateString({
                      year: "numeric",
                      month: "long",
                      day: "numeric",
                    })}
      ...
      <div className="w-full">
        ...
        ></Pagination>
        <ConfirmationModal title="prayer" handleStatus={handleDelete} />
      </div>
    </div>
  );
};

export default EventPrayersTable;
