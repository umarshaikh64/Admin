...
function HafizTable({ data, setIsAscending }) {
  ...
  const handleDelete = () => {
    dispatch(deleteUserById(userid))
      .unwrap()
      .then((res) => {
        infoNotify("Successfully deleted");
      })
      .catch((error) => {
        ...
        <table className="table w-full table-pin-rows table-pin-cols">
          <thead className=" p-0">
            <tr className="font-bold  text-3xl text-blackHigh">
              ...
              {/* date */}
              <th
                className="bg-tablenav text-base text-blackMediumEmp font-normal font-poppins py-4 flex gap-2 cursor-pointer"
              ...
              {currentRows?.map((user, i) => (
                <tr className=" bg-white text-blackSemi" key={i}>
                  ...
                  <td className="py-3">
                    {!isLoading ? (
                      <img
                        src={user?.thumbnailUrl || profileDummy}
