...
import { deleteHadith } from "../../../features/hadith/hadithSlice";
import { errorNotify } from "../../../util/getNotify";
import ConfirmationModal from "../../modals/ConfirmationModal";
import { Pagination } from "../../shared/pagination/Pagination";
import NoData from "../../shared/ui/NoData";

function HadithTable({ data }) {
  ...
  const [hadithId, setHadithId] = useState(null);

  const handleNavigate = (item) => {
    navigate("/hadith-edit", {
      state: {
        payload: item,
        type: "edit",
      },
  ...
  return (
    <div className="h-full flex flex-col justify-between">
      <div className=" overflow-x-auto">
        <table className="table w-full table-pin-rows table-pin-cols">
          ...
          {currentRows?.length === 0 ? (
            ...
            <tbody className="">
              {currentRows?.map((hadith, i) => (
                <tr className=" bg-white text-blackSemi" key={`hadith${i}`}>
                  ...
                  <td className="py-3 text-blackLowEmp text-base font-normal font-poppins">
                    {hadith?.hadithArabic?.length > 30
                      ? hadith?.hadithArabic.slice(0, 30) + "...."
                      : hadith?.hadithArabic}
                  </td>
                  <td className="py-3 text-blackLowEmp text-base font-normal font-poppins">
                    {hadith?.categoryEnglish}
                  </td>
                  <td className="py-3 text-blackLowEmp text-base font-normal font-poppins">
                    {new Date(hadith?.timestamp * 1000).toLocaleDateString({
                      year: "numeric",
                      month: "long",
                      day: "numeric",
                    })}
                  ...
                  <td className="py-3 flex items-center justify-center gap-4">
                    ...
                    <label
                      ...
                      onClick={() => setHadithId(hadith._id)}
                    >
                      <svg
                        width="24"
                        height="24"
      ...
      <div className="w-full">
        ...
        ></Pagination>
        <ConfirmationModal title="hadith" handleStatus={handleDelete} />
      </div>
    </div>
  );
}
