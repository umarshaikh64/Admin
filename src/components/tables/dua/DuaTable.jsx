...
import { deleteDua } from "../../../features/dua/duaSlice";
  ...
  return (
    <div className="h-full flex flex-col justify-between">
              ...
              {currentRows?.map((dua, i) => (
                <tr className=" bg-white text-blackSemi" key={i}>
                  ...
                  <td className="py-3 text-blackLowEmp text-base font-normal font-poppins">
                    {dua?.duaArabic?.length > 30
                      ? dua?.duaArabic?.slice(0, 30) + "..."
                      : dua?.duaArabic}
                  </td>
                  <td className="py-3 text-blackLowEmp text-base font-normal font-poppins">
                    {dua?.duaEnglish?.length > 30
                      ? dua?.categoryEnglish?.slice(0, 30) + "..."
                      : dua?.categoryEnglish}
                  </td>
                  <td className="py-3 text-blackLowEmp text-base font-normal font-poppins">
                    {new Date(dua?.timestamp * 1000).toLocaleDateString({
                      year: "numeric",
                      month: "long",
                      day: "numeric",
                    })}
                  ...
                  <td className="py-3 flex items-center justify-center gap-4">
                    <button type="button" onClick={() => handleNavigate(dua)}>
                      <svg
                        width="24"
                        height="24"
                        viewBox="0 0 24 24"
      ...
      <div className="w-full">
        ...
        ></Pagination>
        <ConfirmationModal title="dua" handleStatus={handleDelete} />
      </div>
    </div>
  );
};
