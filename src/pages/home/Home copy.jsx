...
import { getAllUsers, getChartsData } from "../../features/users/usersSlice";
...
export default function Home() {
  ...
  const chartsData = useSelector((state) => state?.users?.chartsData);
  ...
  useEffect(() => {
    ...
    const totalDonors = users?.filter(
      (user) => parseInt(user?.totalDonation) > 0 && user?.totalDonation != ""
    ).length;
    const totalDonation = users?.reduce((acc, user) => {
      if (user?.totalDonation) {
        return acc + parseInt(user?.totalDonation);
      }
      return acc;
    }, 0);
    setData([
  ...
  return (
    <section className="h-full relative p-6">
      <div className="flex flex-col gap-8">
        ...
        <div className="w-full bg-white p-6 rounded-2xl">
          ...
          <div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 items-stretch gap-8 mt-10">
              {/* card one  */}
              {data?.map((item, i) => (
